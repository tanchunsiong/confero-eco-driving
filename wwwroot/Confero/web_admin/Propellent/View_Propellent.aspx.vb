...
Partial Class Propellent_View_Propellent
    ...
    Sub bindData()
        ...
        If Not txtSearch.Text.Trim.Equals("") Then
            objArrayList = objDBPropellent.Search_By_Propellent(lstSelect.SelectedValue, txtSearch.Text)
            lblSearch.Text = "Search Record: " + objArrayList.Count.ToString
        Else
            objArrayList = objDBPropellent.Get_All_Propellent
    ...
    Protected Sub grdData_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles grdData.ItemCommand
        ...
        ElseIf (e.CommandName.Equals("update")) Then
            Response.Redirect("~/web_admin/Propellent/Add_Propellent.aspx?for=update&id=" & e.Item.Cells(0).Text)
        End If
    End Sub
    ...
    Protected Sub btnExport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnExport.Click
        ...
        If txtSearch.Text.Trim.Equals("") Then
            query = "SELECT tblPropellent.*, tblPropellentType.description FROM tblPropellent INNER JOIN tblPropellentType ON tblPropellent.propellentTypeID = tblPropellentType.PropellentTypeID"
        Else
            query = "SELECT tblPropellentType.description, tblPropellent.* " & _
                    "FROM tblPropellent INNER JOIN " & _
                    "tblPropellentType ON tblPropellent.propellentTypeID = tblPropellentType.PropellentTypeID " & _
                    "where " & lstSelect.SelectedValue & " like '%" & txtSearch.Text & "%'"
        End If
